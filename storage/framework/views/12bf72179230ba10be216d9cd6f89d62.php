...
<script>
  document.querySelector("head > meta[name='theme-color'][content='#0e4688']").setAttribute("content", "<?php echo e($n_palette_auth_1); ?>")
</script>
...
<style id="nebula-authentication-theme">
  ...
  @keyframes backdrop {
    0%   {scale: calc(.1 +<?php if($n_auth_background_appearance == "1"): ?> 2.4 <?php else: ?> 1 <?php endif; ?>)}
    100% {scale: <?php if($n_auth_background_appearance == "1"): ?> 2 <?php else: ?> 1 <?php endif; ?>}
  }

  <?php if($n_auth_customlogo != ""): ?>
    .LoginFormContainer__Container-sc-cyh04c-0.cEWvSE .LoginFormContainer___StyledH-sc-cyh04c-1.hpqfJy {
      content: url("<?php echo e($n_auth_customlogo); ?>");
      border-radius: 10px;
      padding: 0;
      height: 65px;
      max-width: 100%;
  ...
  .nebula-auth-wallpaper { 
    z-index: 3;
    overflow: hidden;
    <?php if($n_auth_background_image == ""): ?>background-color: var(--authA);
    <?php else: ?> background: url("<?php echo e($n_auth_background_image); ?>") no-repeat; background-color: #000;<?php endif; ?>
    ...
    top: 0; left: 0;
    position: fixed;
    <?php if($n_auth_background_appearance == "1"): ?>filter: blur(50px);scale: 2;<?php endif; ?>
    <?php if($n_auth_background_appearance == "1"): ?>filter: blur(50px);<?php endif; ?>
    <?php if($n_auth_background_appearance == "2"): ?>opacity: 0.6;<?php endif; ?>
...
<?php endif; ?><?php /**PATH /var/www/pterodactyl/resources/views/blueprint/extensions/nebula/wrapper/theme/auth.blade.php ENDPATH**/ ?>